...
export const mutations = {
  ...
  createThread: async (thread: any) => {
    try {
      ...
      const result = await convexServer.mutation(api.threads.createThread, threadData);
  ...
  createMessage: async (message: any) => {
    try {
      ...
      const result = await convexServer.mutation(api.messages.upsertMessage, messageData);
  ...
  createStream: async (stream: any) => {
    try {
      ...
      const result = await convexServer.mutation(api.streams.createStream, streamData);
  ...
  updateThreadWithId: async (thread: { id: string } & any) => {
    try {
      ...
      if (typeof updates.title !== 'undefined') formattedUpdates.title = updates.title;
      if (typeof updates.model !== 'undefined') formattedUpdates.model = updates.model;
      if (typeof updates.status !== 'undefined') formattedUpdates.status = updates.status;
      if (typeof updates.pinned !== 'undefined') formattedUpdates.pinned = updates.pinned;

      await convexServer.mutation(api.threads.updateThreadWithExternalId, formattedUpdates);
      return { ...thread };
    } catch (error) {
