...
export default function HelpPage() {
  ...
  const helpArticles = [
    ...
    { title: "Managing your Account & Subscription", category: "Account", icon: Shield },
    { title: "How to use DeepThink and Web Search", category: "Features", icon: BookOpen },
    { title: "Troubleshooting common issues", category: "Support", icon: HelpCircle },
  ];
      ...
      <div className="w-full max-w-[900px] bg-[#1c1c1e] rounded-2xl border border-zinc-800/50 shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="p-8 md:p-12 bg-gradient-to-b from-blue-600/10 to-transparent border-b border-zinc-800/50 relative">
          ...
          <div className="max-w-2xl mx-auto text-center space-y-6">
            ...
            <p className="text-zinc-400 text-sm">Advice and answers from the Fuseion Team</p>
          ...
          {helpArticles.map((article, idx) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest">{article.category}</p>
                <h3 className="text-sm font-semibold text-zinc-200 mt-0.5 truncate">{article.title}</h3>
              </div>
        ...
        <div className="p-8 border-t border-zinc-800/50 bg-zinc-900/20 text-center">
          <div className="flex flex-col items-center gap-4">
             <div className="flex -space-x-3">
               {[1,2,3].map(i => (
                 <div key={i} className="h-8 w-8 rounded-full border-2 border-[#1c1c1e] bg-zinc-800 flex items-center justify-center shadow-lg">
                   <div className="h-full w-full rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 opacity-50" />
                 </div>
               ))}
             </div>
             <p className="text-sm text-zinc-400">Join our community to connect with other users and get fast support.</p>
